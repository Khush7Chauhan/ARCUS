...
app.post("/deploy", async (req, res) => {
    // NEW: Accept projectId so deployments are linked together
    const { repoUrl, projectId, customDomain } = req.body;
    const id = generate(); 
    ...
    // NEW: Store project metadata so deploy-service knows what domain to update
    if (projectId) {
        await publisher.hSet("project:latest-id", projectId, id);
        await publisher.hSet("deployment:project", id, projectId);
    }
    // Fallback: allow passing domain directly for one-off updates
    if (customDomain) {
        await publisher.set(`build:domain:${id}`, customDomain);
    }
